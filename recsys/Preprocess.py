...
class preprocess(Dataset):
    ...
    # 태그 데이터를 사용해 임베딩을 학습 실시
        ...
        tag = pd.Series("<sos> " + total_df['tag']+" <end>")
        tag.to_csv(model_dir+"summary_text_corpus.csv")

        # skip-gram 기반 임베딩 training
        kor2vec = Kor2Vec(embed_size=embed_size_tune) # embed_size : 임베딩 벡터의 2번째 차원(차원 수) 64차원
...
img, tag = reveiw_train_data[0]

print(f"img shape : {img.shape}") # ([1, 3, 224, 224]) - [배치, 채널, 가로, 세로]
print(f"tag : {tag.shape}") # ([20, 64])) - [seq_len, hidden_size]
